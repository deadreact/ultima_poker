...
struct binary_istream {
	...
	// for 64-bit ints
	template<typename T>
    void read(T &v, YAS_ENABLE_IF_IS_ANY_OF(T, std::int64_t, std::uint64_t, long long int, long long unsigned int)) {
		YAS_THROW_ON_READ_ERROR(sizeof(v), !=, is.read(&v, sizeof(v)));
...
struct binary_ostream {
	...
	// for 64-bit ibts
	template<typename T>
    void write(T v, YAS_ENABLE_IF_IS_ANY_OF(T, std::int64_t, std::uint64_t, long long int, long long unsigned int)) {
